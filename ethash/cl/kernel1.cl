	...
	for (uint tid = 0; tid < THREADS; tid++) {
		...
		for (uint a = 0; a < ACCESSES; a += 8) {
			...
			for (uint i = 0; i != 8; ++i) {
				...
				mix = FNV(mix, g_dag[share->uints[0] >> 1].uint8s[thread_id]);
				barrier(CLK_LOCAL_MEM_FENCE);
			}
		}
...
#define ROTL64_1_1(x, y) as_uint2(rotate(as_ulong(x), (ulong)(y)))
#define ROTL64_2_1(x, y) ROTL64_1(x, (y) + 32)

#define KECCAKF_1600_RND(a, i, outsz) do { \
    const uint2 m0 = a[0] ^ a[5] ^ a[10] ^ a[15] ^ a[20] ^ ROTL64_1_1(a[2] ^ a[7] ^ a[12] ^ a[17] ^ a[22], 1);\
    const uint2 m1 = a[1] ^ a[6] ^ a[11] ^ a[16] ^ a[21] ^ ROTL64_1_1(a[3] ^ a[8] ^ a[13] ^ a[18] ^ a[23], 1);\
    const uint2 m2 = a[2] ^ a[7] ^ a[12] ^ a[17] ^ a[22] ^ ROTL64_1_1(a[4] ^ a[9] ^ a[14] ^ a[19] ^ a[24], 1);\
    const uint2 m3 = a[3] ^ a[8] ^ a[13] ^ a[18] ^ a[23] ^ ROTL64_1_1(a[0] ^ a[5] ^ a[10] ^ a[15] ^ a[20], 1);\
    const uint2 m4 = a[4] ^ a[9] ^ a[14] ^ a[19] ^ a[24] ^ ROTL64_1_1(a[1] ^ a[6] ^ a[11] ^ a[16] ^ a[21], 1);\
    \
    ...
    a[24] ^= m3; \
    \
    a[1] = ROTL64_2_1(a[6], 12);\
    a[6] = ROTL64_1_1(a[9], 20);\
    a[9] = ROTL64_2_1(a[22], 29);\
    a[22] = ROTL64_2_1(a[14], 7);\
    a[14] = ROTL64_1_1(a[20], 18);\
    a[20] = ROTL64_2_1(a[2], 30);\
    a[2] = ROTL64_2_1(a[12], 11);\
    a[12] = ROTL64_1_1(a[13], 25);\
    a[13] = ROTL64_1_1(a[19],  8);\
    a[19] = ROTL64_2_1(a[23], 24);\
    a[23] = ROTL64_2_1(a[15], 9);\
    a[15] = ROTL64_1_1(a[4], 27);\
    a[4] = ROTL64_1_1(a[24], 14);\
    a[24] = ROTL64_1_1(a[21],  2);\
    a[21] = ROTL64_2_1(a[8], 23);\
    a[8] = ROTL64_2_1(a[16], 13);\
    a[16] = ROTL64_2_1(a[5], 4);\
    a[5] = ROTL64_1_1(a[3], 28);\
    a[3] = ROTL64_1_1(a[18], 21);\
    a[18] = ROTL64_1_1(a[17], 15);\
    a[17] = ROTL64_1_1(a[11], 10);\
    a[11] = ROTL64_1_1(a[7],  6);\
    a[7] = ROTL64_1_1(a[10],  3);\
    a[10] = ROTL64_1_1(tmp,  1);\
    \
...
static void SHA3_512_2(uint2 *s)
{
    uint2 st[25];
