...
static void keccak_f1600_round(ulong *a, const uint r, const uint outsz) {
  const ulong m0 = a[0] ^ a[5] ^ a[10] ^ a[15] ^ a[20] ^
                   ROTL64_1(a[2] ^ a[7] ^ a[12] ^ a[17] ^ a[22], 1);
  const ulong m1 = a[1] ^ a[6] ^ a[11] ^ a[16] ^ a[21] ^
                   ROTL64_1(a[3] ^ a[8] ^ a[13] ^ a[18] ^ a[23], 1);
  const ulong m2 = a[2] ^ a[7] ^ a[12] ^ a[17] ^ a[22] ^
                   ROTL64_1(a[4] ^ a[9] ^ a[14] ^ a[19] ^ a[24], 1);
  const ulong m3 = a[3] ^ a[8] ^ a[13] ^ a[18] ^ a[23] ^
                   ROTL64_1(a[0] ^ a[5] ^ a[10] ^ a[15] ^ a[20], 1);
  const ulong m4 = a[4] ^ a[9] ^ a[14] ^ a[19] ^ a[24] ^
...
#endif
__kernel void
search(__global volatile struct search_results_t *restrict g_output,
       __constant hash32_t const *g_header, __global hash128_t const *g_dag1,
       __global hash128_t const *g_dag2, uint dag_size, ulong start_nonce,
       ulong target) {
#ifdef FAST_EXIT
  if (g_output->abort) {
    return;
...
#pragma unroll
      for (uint i = 0; i != 16; ++i) {
        ...
        for (uint x = 0; x < 16; x++) {
          mix[x] =
              FNV(mix[x], g_dag[share->uints[0] >> 1].uint16s[thread_id][x]);
        }
        barrier(CLK_LOCAL_MEM_FENCE);
      }
    }
    ...
    share->uint4s[thread_id] =
        (uint4)(FNV_REDUCE(mix.s0123), FNV_REDUCE(mix.s4567),
                FNV_REDUCE(mix.s89ab), FNV_REDUCE(mix.scdef));
    barrier(CLK_LOCAL_MEM_FENCE);
...
__kernel void generate_dag_item(uint start, __global hash64_t const *g_light,
                                __global hash64_t *g_dag1,
                                __global hash64_t *g_dag2) {
  uint node_id = start + get_global_id(0);

  __global hash64_t *g_dag;

  if (node_id > DAG_SIZE * 2)
    return;
  ...
  for (uint i = 0; i != ETHASH_DATASET_PARENTS; ++i) {
    uint parent_index =
        FNV(node_id ^ i, dag_node.uints[i % NODE_SIZE]) % LIGHT_SIZE;
