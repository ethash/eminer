...
func (c *OpenCLMiner) generateDAGOnDevice(d *OpenCLDevice) error {
	...
	d.dagBuf, err = d.ctx.CreateEmptyBuffer(cl.MemReadWrite, c.dagSize)
	if err != nil {
	...
	err = dagKernel.SetArg(2, d.dagBuf)
	if err != nil {
		return fmt.Errorf("set arg failed %v", err)
	}
...
func (c *OpenCLMiner) Seal(stop <-chan struct{}, deviceID int, onSolutionFound func(bool, uint64, []byte, uint64)) error {
	...
	worker := func(s *search) {
		...
		for !c.stop {
			...
			err = d.searchKernel.SetArg(3, s.startNonce)
			if err != nil {
			...
			err = cl.WaitForEvents([]*cl.Event{event})
			if err != nil {
				d.logger.Error("Error in seal wait events", "error", err.Error())
				continue
			}
